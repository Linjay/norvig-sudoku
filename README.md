...
内置完整题库 112 道（top95 + hardest 全部题目，另含挖空生成的简单题；
难度按求解器实际猜测次数标定：简单=0 次猜测、中等 1~5、困难 6~30、
专家 30+），列表按难度从易到难排序并支持快速筛选。选题求解后可逐步
回放约束传播、唯一候选、唯一位置、猜测与回溯的完整路径，支持播放/
单步/调速/关键步与全部步两档粒度，也可粘贴自定义 81 字符题串。
